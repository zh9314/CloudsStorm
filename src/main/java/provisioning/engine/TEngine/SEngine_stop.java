    ...
    public void run() {
        try {
            ...
            if (!((SEngineCoreMethod) sEngine).stop(subTopologyInfo, credential, database)) {
                logger.warn("Some VMs of sub-topology '" + subTopologyInfo.topology + "' cannot be stopped! Delete!");
                if (!((SEngineCoreMethod) sEngine).delete(subTopologyInfo, credential, database)) {
                    logger.error("Some VMs of sub-topology '" + subTopologyInfo.topology + "' cannot be deleted!");
