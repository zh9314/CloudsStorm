...
public abstract class SubTopology implements SubTopologyMethod, TopologyMethod, Cloneable {
    ...
    /**
     * Indicate what the Cloud of this sub-topology is. Indicate the cloud
     * provider of this sub-topology. For instance, ec2, exogeni and egi.
     */
    @JsonIgnore
    public String cloudProvider;

    /**
     * Get this information from the SubTopologyInfo, in order to check and get
     * the member variables in the sub-class of this SubTopology.
     */
    @JsonIgnore
    public String subTopologyClass;

    /**
     * Specify the S-Engine Class to handle this sub-topology. Useful for
     * extension to currently unsupported Clouds. If it is null, than S-Engine
     * for current supported Clouds will be loaded by default.
     */
    ...
    /**
     * This is the common format checking, no matter where the sub-topology
     * comes from. This is invoked at the beginning of the
     * formatChecking(String) method in sub classes. The checking items
     * includes: <br/>
     * 1. Make sure that there is 'VMs' defined in the sub-class. 2. Checking
     * all the node names whether in a correct format, which should not exceed
     * 20 characters and do not contain '.' or '_'. 3. All the node names should
     * be different. <br/>
     * 4. The VM cannot have public address, if the status of the sub-topology
     * is 'fresh'. 5. The VM must have public address, if the status of the
     * sub-topology is 'running'.
     *
    ...
    /**
     * Write the ssh key pair to files, if needed. This key pair is used for
     * accessing the sub-topology in this Data center.
     *
     * @return
     */
    @JsonIgnore
